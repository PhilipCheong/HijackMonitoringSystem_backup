...
namespace HijackMonitoringApplication.Controllers
{
	public class PerformancesResultsController : BaseController
	{
		...
		{
			var startDate = Convert.ToDateTime(dateTime).AddMinutes(-5);
			var endDate = Convert.ToDateTime(dateTime).AddMinutes(5);

			var query = performanceDataService.Find(s => s.Url.Equals(domainName)).ToList();

            var currentQuery = query.Where(s => s.TestTime.ToLocalTime() == Convert.ToDateTime(dateTime)).ToList();

			return JsonConvert.SerializeObject(currentQuery.Where(s => s.Province.Contains(province)));
		}
		...
		{
			var query = performanceDataService.Find(s => s.Url.Equals(domainName) && s.TestTime >= Convert.ToDateTime(startDate) && s.TestTime < Convert.ToDateTime(endDate)).ToList();
